...
>(({ className, children, ...props }, ref) => {
  ...
  // Force inline rendering for Fix-OS - ensure generative UI components are always visible
  // We check for both message.renderedComponent and message.component to be ultra-safe
  const hasComponent = message.renderedComponent || message.component;
  ...
  return (
    <div
      ref={ref}
      className={cn("w-full mt-4 border-t border-zinc-800/50 pt-4", className)}
      data-slot="message-rendered-component-area"
      {...props}
    >
      {message.renderedComponent}
      {!message.renderedComponent && message.component && (
        <div className="text-xs text-zinc-500 italic">
          [Component {message.component.componentName} available but not rendered]
        </div>
      )}
    </div>
  );
